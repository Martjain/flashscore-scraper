...
export const DEFAULT_SMOKE_REPORT_PATH = ".planning/artifacts/smoke/latest.json";

const HISTORY_FILE_PREFIX = "smoke-";

const toHistoryTimestamp = (date = new Date()) =>
  date.toISOString().replace(/[:.]/g, "-");

const resolveReportPaths = (reportPath = DEFAULT_SMOKE_REPORT_PATH) => {
...
export const writeSmokeReports = async (
  report,
  reportPath = DEFAULT_SMOKE_REPORT_PATH
) => {
...
export const pruneSmokeHistory = async (
  reportPath = DEFAULT_SMOKE_REPORT_PATH,
  keepCount = 30
) => {
...
export const persistSmokeReport = async (report, options = {}) => {
  const reportPath = options.reportPath || DEFAULT_SMOKE_REPORT_PATH;
  const keepHistory = Number.isInteger(options.keepHistory)
    ? options.keepHistory
    : 30;
  const { latestPath, historyPath } = await writeSmokeReports(report, reportPath);
  const { removed } = await pruneSmokeHistory(reportPath, keepHistory);
